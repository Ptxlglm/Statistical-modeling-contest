...
# 数据准备
metal_cols = ['Zn', 'Ni', 'Cu', 'Pb', 'Fe']
case_metal = df[df['group(2对照,1病例)']==1][metal_cols].mean()  # 病例组均值
control_metal = df[df['group(2对照,1病例)']==0][metal_cols].mean()  # 对照组均值

# 创建绘图数据框
plot_df = pd.DataFrame({
    'Metal': metal_cols*2,
    'Concentration': pd.concat([case_metal, control_metal]),
    'Group': ['病例组']*5 + ['对照组']*5
})

# 设置颜色映射（与热力图保持一致）
cmap = plt.get_cmap('coolwarm')
case_color = cmap(0.9)  # 红色系
control_color = cmap(0.1)  # 蓝色系
...
ax = sns.barplot(
    ...
    hue='Group',
    data=plot_df,
    palette=[case_color, control_color],
    edgecolor='black',
    linewidth=1.2
)
...
# 图形修饰
plt.title('重金属浓度均值对比（标准化后）', fontsize=14, pad=20)
plt.xlabel('重金属元素', fontsize=12)
plt.ylabel('标准化浓度', fontsize=12)
...
norm = plt.Normalize(plot_df['Concentration'].min(), plot_df['Concentration'].max())
sm = plt.cm.ScalarMappable(cmap='coolwarm', norm=norm)
sm.set_array([])
plt.colorbar(sm, ax=ax, label='浓度梯度', pad=0.02)
...
# # 数据整理：将三个变量从宽格式转为长格式
# df_melt = df.melt(id_vars=['group(2对照,1病例)'],
#                   value_vars=['age', 'Zn', 'Ni'],
#                   var_name='variable',
#                   value_name='value')
#
# # 创建FacetGrid分面网格
# g = sns.FacetGrid(df_melt, col='variable', col_wrap=3,
#                   hue='group(2对照,1病例)', palette={0:'blue', 1:'orange'},
#                   sharex=False, sharey=False, height=4)
#
# # 每个子图绘制核密度曲线
# g.map(sns.kdeplot, 'value', fill=True, alpha=0.6)
# g.add_legend(title='Group', labels=['Control (0)', 'Case (1)'])
...
#     search_spaces={
#         'n_estimators': Integer(100, 500),        # 整数型参数空间
#         'max_depth': Integer(3, 30),              # 原None改为合理数值范围
#         'min_samples_split': Integer(2, 10),       # 原[2,5]改为范围
#         'class_weight': Categorical([None, 'balanced'])  # 类别型参数
#     },
#     n_iter=30,  # 优化迭代次数（原网格搜索7.5倍效率）
#     cv=5,
#     scoring='roc_auc',
#     n_jobs=-1,
#     random_state=42
